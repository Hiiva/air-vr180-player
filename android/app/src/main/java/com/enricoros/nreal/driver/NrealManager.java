...
public class NrealManager {
  ...
  public interface Listener {
    ...
    /** Runs synchronously on the USB reader thread. The sample is valid only during this call. */
    void onImuSample(ImuDataRaw sample);
  ...
  private final NrealDeviceThread.ThreadCallbacks mReaderCallbacks = new NrealDeviceThread.ThreadCallbacks() {
    ...
    public void onNewData(ImuDataRaw data) {
      // Fusion must see every sample, before the UI's latest-only coalescing discards any.
      listener.onImuSample(data);
